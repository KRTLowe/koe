...
impl Tool for ScreenshotTool {
    ...
    fn execute(&self, args: &Value) -> ToolResult {
        ...
        let crop_h = args.get("height").and_then(|v| v.as_u64()).map(|v| v as u32);

        let do_crop = crop_x.is_some() && crop_y.is_some() && crop_w.is_some() && crop_h.is_some();
        if do_crop {
            log::info!("[ScreenshotTool] region crop: x={:?} y={:?} w={:?} h={:?}", crop_x, crop_y, crop_w, crop_h);
        }
        ...
        let result = {
            ...
            log::info!("[ScreenshotTool] screen count={}, using first screen {:?}",
                screens.len(), screen);

            match screen.capture() {
                Ok(image) => {
                    log::info!("[ScreenshotTool] capture succeeded: {}x{}", image.width(), image.height());
                    let dyn_img = DynamicImage::from(image);

                    let final_img = if do_crop {
                        let (x, y, w, h) = (
                            crop_x.unwrap(), crop_y.unwrap(),
                            crop_w.unwrap(), crop_h.unwrap(),
                        );
                        // 裁剪边界安全：不超出图像范围
                        ...
                        log::info!("[ScreenshotTool] cropping to x={} y={} w={} h={}", crop_x, crop_y, crop_w, crop_h);
                    ...
                    match final_img.write_to(&mut png_buf, ImageFormat::Png) {
                        Ok(_) => {
                            ...
                            log::info!("[ScreenshotTool] PNG encoded: {} bytes, filename={}", png_data.len(), filename);
        ...
        log::info!("[ScreenshotTool] execute end: elapsed={:?}, is_error={}, upload={:?}",
            start.elapsed(), result.is_error, result.upload_path);
        result
    }
}
