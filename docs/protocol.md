...
- 服务端在处理客户端上传的二进制文件帧时，也会把该客户端视为活跃并刷新在线心跳时间。
- 客户端在长时间上传文件时，会周期性发送协议层 `Ping` 和应用层 `heartbeat`，避免上传循环期间被误判为离线。
...
对于长时间上传，客户端会在上传循环中周期性发送保活帧；服务端收到后继续维持在线状态，不要求上传必须拆回普通心跳轮询后才能算活跃。
